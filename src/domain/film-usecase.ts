...
export class FilmUsecase {
    ...
    async getFilm(): Promise<{ author: string, sortie: Date }[] | null> {
        ...
        const films = await repo
            ...
            .select(['film.title AS title','film.description AS description','film.duration AS duration','film.genre AS genre','film.author AS author','film.affectedRoom AS affectedRoom']) 
            .getRawMany();
